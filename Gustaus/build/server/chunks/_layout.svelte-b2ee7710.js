import { c as create_ssr_component, v as validate_component, a as add_attribute, e as each, m as missing_component, b as escape } from './ssr-58f9b3e9.js';
import { a as addRuntimeToContext, g as getRuntimeFromContext, p as page } from './context-b6dcdab0.js';
import { c as getRuntimeFromData } from './utils-fdb1ab73.js';
import { a as addRuntimeToGlobalThis, o as objectMerge, p as position, t as toast, D as DEFAULT_ANIMATION } from './Toasts.svelte_svelte_type_style_lang-96cd2bd3.js';
import { s as subscribe, a as set_store_value } from './utils2-05c95967.js';
...
const css$1 = {
  code: "nav.svelte-dqruvm{padding:0.5rem 1rem;margin-bottom:2rem ;border-bottom:1px solid var(--primary);background-color:var(--form-element-focus-color)}a.svelte-dqruvm{display:flex;justify-content:space-evenly;padding:0.5rem;height:min-content;margin:auto}a[aria-current='true'].svelte-dqruvm{color:var(--inverse);border-radius:5px;border-left:1px solid var(--primary);border-right:1px solid var(--primary)}img.svelte-dqruvm{height:50px;margin:auto 0}button.svelte-dqruvm{border:none;background:none;padding:0;cursor:pointer;margin:0;width:fit-content}",
  map: null
};
const Navbar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $page, $$unsubscribe_page;
  $$unsubscribe_page = subscribe(page, (value) => $page = value);
  const { i, language, switchLanguage } = getRuntimeFromContext();
  let nav_items = [
    { "link": "/", "title": i("nav.home") },
    { "link": "/menu", "title": i("nav.menu") },
    {
      "link": "/reserve",
      "title": i("nav.reserve")
    },
  ...
  $$unsubscribe_page();
  return `<nav class="svelte-dqruvm"><img src="/logo.png" alt="logo" class="svelte-dqruvm"> ${each(nav_items, (route) => {
    return `<a${add_attribute("href", route.link, 0)}${add_attribute("aria-current", $page.route.id === route.link, 0)} class="svelte-dqruvm">${escape(route.title)}</a>`;
  })} <button type="menu" ${language == "en" ? "hidden" : ""} class="svelte-dqruvm"><img src="/en.png" alt="english icon" draggable="false" class="svelte-dqruvm"></button> <button ${language == "ru" ? "hidden" : ""} class="svelte-dqruvm"><img src="/ru.png" ${language == "ru" ? "hidden" : ""} alt="russian icon" draggable="false" class="svelte-dqruvm"></button> </nav>`;
});
const css = {
  code: "main.svelte-10gyfu4{margin-top:3rem}@media(min-width: 780px){main.svelte-10gyfu4{margin:0 6rem}}main h1, main h2, main h3, main h4, main h5, main h6, main p{padding:0 0.5rem;font-weight:200}:root{--svoast-bar-width:3px;--svoast-font-size:19px;--svoast-bg:#11191f}",
  map: null
};
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$result.css.add(css);
  return `${language || !referenceLanguage ? ` ${validate_component(Navbar, "Navbar").$$render($$result, {}, {}, {})} <main class="svelte-10gyfu4">${slots.default ? slots.default({}) : ``}</main> ${validate_component(Toasts, "Toasts").$$render($$result, { position: "bottom-center" }, {}, {})} ` : ``}`;
});

export { Layout as default };
//# sourceMappingURL=_layout.svelte-b2ee7710.js.map
