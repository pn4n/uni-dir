...
async function setupPlugins(args) {
  ...
  for (let i = 0; i < args.config.plugins.length; i++) {
    try {
      ...
      const config2 = await plugin?.config({ ...mergedConfig });
      deepmergeInto(mergedConfig, config2);
    } catch (error) {
...
const messageReferenceSchema = objectType({
  messageId: stringType(),
  position: positionSchema
});
const zodIdeExtensionConfigSchema = objectType({
  /**
...
class ParseConfigException extends Error {
  constructor() {
    super(...arguments);
...
function hasSetupAResourcePlugin(config2) {
  if (!config2.readResources || !config2.writeResources) {
...
function referenceLanguageMustBeInLanguages(config2) {
  if (!config2.languages.includes(config2.referenceLanguage)) {
    throw new ParseConfigException(`The reference language "${config2.referenceLanguage}" must be included in the list of languages.`);
  }
}
...
async function languagesMatch(config2, resources) {
  const languages = resources.map((resource) => resource.languageTag.name);
  const areEqual = languages.sort().join(",") === config2.languages.sort().join(",");
  if (areEqual === false) {
...
async function roundtripTest(config2, initialResources) {
  const commonErrorMessage = "A roundtrip test of the readResources and writeResources functions failed:\n";
  await config2.writeResources({ config: config2, resources: initialResources });
  const readResourcesAgain = await config2.readResources({ config: config2 });
...
async function setupConfig(args) {
  ...
  let config2 = await args.module.defineConfig(args.env);
  const [configWithPlugins, pluginErrors] = await setupPlugins({ config: config2, env: args.env });
  config2 = configWithPlugins;
  const [parsedConfig, testConfigException] = await parseConfig({ config: config2 });
...
const initInlangEnvironment = async () => {
...
let config;
const initState = async (module) => {
  if (!config && true) {
    try {
      const { languages, referenceLanguage, resources } = await import('./_virtual_inlang-static-4b529d8e.js');
      config = {
        referenceLanguage,
        languages,
        readResources: async () => resources,
        writeResources: async () => void 0
      };
    } catch {
    }
  }
...
let _resources = [];
const reloadResources = async () => _resources = await config?.readResources({ config }) || [];
const getResource = (language) => _resources.find(({ languageTag: { name } }) => name === language);
const addRuntimeToLocals = (locals, runtime) => locals[inlangSymbol] = runtime;
const getRuntimeFromLocals = (locals) => locals[inlangSymbol];

export { addRuntimeToLocals as a, getResource as b, getRuntimeFromLocals as g, initState as i, reloadResources as r };
//# sourceMappingURL=state-53e380b5.js.map
